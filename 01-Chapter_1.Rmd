...
In alternative, if you want to run just one line of code, you can place the cursor at the end of the line that you wish to execute and then press *Run*. If you want to run all the code in a script, you can use the *Source* button. There are two options, *Source* and *Source with Echo*. If you just *Source* your script, you run your entire script but the code and the output of your code will not be printed in the console. If you *Source with Echo*, your code will be executed (just like with *Source*) but this time you will see your code and the output of your code printed in the console. It is up to you to decide when to use one or the other.

## Basic operations

We now start by looking at some very simple operations that can be performed in R. Before we begin, however, I would like to make a disclaimer. The list of operations and functions that we are going to see is not complete. When I say not complete I simply mean that what we are going to see in this section are by no means all the functions available in the R language. I have decided to focus our attention on a number of operations and functions that I believe are useful to our quantitative methods course for economists and that build the basis for more complex analysis. For those who want to learn more about R, the literature available nowadays is really large. I will provide some references as we go through the explanations in the various sections.^[A good book to start with the study of R is *The Art of R Programming* by Normal Matloff [@Matloff] but there are many many books available out there without counting other resources like YouTube videos and online courses.] Now back to our script. 
...
We can also generate random numbers from a normal distribution specifying mean and standard deviation (more on the normal distribution in Part II). The `set.seed` function allows us to generate the same "random" numbers. This means that if you and I have the same seed, we will generate the same set of random numbers.^[This function is essential if we want someone else to be able to replicate the results of our simulations. This might be the case when we want to share our work with our fellow students or with our thesis supervisor.]
